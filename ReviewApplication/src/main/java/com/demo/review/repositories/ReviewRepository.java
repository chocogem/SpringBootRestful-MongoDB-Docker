...
public interface ReviewRepository extends JpaRepository<Review, Integer>{
	...
	@Query(value ="SELECT UUID() AS guid,reviewID,replace(group_concat(review_text),replace(:text,'%',''),concat('%keyword%',replace(:text,'%',''),'%/keyword%')) AS review_text,1 AS seq from (select * FROM REVIEW WHERE review_text like %:text% order by reviewID,seq)  group by reviewID",nativeQuery = true)
	...
	@Modifying
	@Query(value =" DELETE from Review r WHERE r.reviewID = :id ")
